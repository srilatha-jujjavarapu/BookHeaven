...
const SeeUserData = ({userDivData, userDiv, setUserDiv}) => {
  return (
    ...
    <div className={`${userDiv} top-0 left-0 h-screen w-full flex items-center justify-center`}>
        <div className='bg-white rounded p-4 w-[80%] md:w-[50%] lg:w-[40%] text-zinc-900'>
            <div className='flex items-center justify-between'>
                <h1 className='text-2xl font-semibold'>User Information</h1>
                <button className='font-bold text-red-700 me-0' onClick={()=> setUserDiv("hidden")}><ImCross /></button>
            </div>
            <div className='mt-2'>
                <label htmlFor=''>Username : <span className='font-semibold'>{userDivData.username}</span></label>
            </div>
            <div className='mt-2'>
